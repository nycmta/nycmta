...
- **Daily trends** show consistent ridership with clear weekly periodicity—weekday peaks and weekend dips. The 7-day rolling average smooths out day-to-day variation, revealing a stable baseline of ~2.5M daily rides.
- **Holiday dips** are clearly visible, with ridership dropping sharply during July 4th, Thanksgiving, and Christmas/New Year periods—reflecting reduced commuter activity when offices close and residents leave the city.
- **Hourly patterns** reveal the classic dual-peak commuter signature on weekdays (8-9 AM and 5-6 PM rush hours), while weekends show a single midday plateau reflecting leisure travel. Weekday ridership exceeds weekend by roughly 2x during peak hours.
- **Top stations** are dominated by Midtown hubs (Penn Station, Times Square, Grand Central) that serve as major transit interchanges, followed by key employment centers and tourist destinations.

**Download script:** [`download_full_ridership_monthly.R`](data/download_full_ridership_monthly.R)
...
- **Temperature** follows expected seasonal patterns with summer highs reaching 90°F+ and winter lows near freezing. The daily variation is substantial (20-30°F swings common), creating natural experiments for studying temperature effects on ridership.
- **Precipitation** is episodic rather than seasonal—significant rain events (≥1 inch) occur year-round but cluster in spring and fall. These storm days represent potential ridership disruptions.
- **Seasonal summary** confirms summer has the most heat days (≥90°F) while storm days distribute across spring, summer, and fall. Winter shows fewer extreme events but presents its own challenges (snow, cold).

**Download script:** [`download_noaa_weather.R`](data/download_noaa_weather.R)
...
- **Monthly distribution** reveals strong seasonality—event counts peak in summer and early fall (June-October) when outdoor activities flourish, then drop sharply in winter months. Manhattan consistently hosts the largest share of citywide events.
- **Event types** in Manhattan are dominated by street events (street fairs, festivals) and block parties, followed by athletic events (marathons, races). Parades, while fewer in number, often represent the largest individual events (NYC Marathon, Pride March, etc.).
- **Borough comparison** shows Manhattan's event density far exceeds other boroughs, reflecting its role as NYC's commercial and cultural center—important context for our station-level impact analysis.

**Download script:** [`download_historical_events_monthly.R`](data/download_historical_events_monthly.R)
...
- **Spatial concentration** is striking—the highest-ridership stations (bright yellow/orange) cluster in Midtown Manhattan (34th-59th Streets), forming a dense core of transit activity around Penn Station, Times Square, and Grand Central.
- **North-South gradient** shows ridership declining as you move toward Upper Manhattan (Harlem, Washington Heights) and the Financial District edges, reflecting the concentration of employment and tourism in Midtown.
- **Station density** varies considerably—Midtown has closely spaced stations serving office towers, while residential Upper Manhattan has wider spacing. This spatial structure influences how events and weather may differentially impact neighborhoods.

*Note: Station coordinates are extracted directly from the ridership data—no separate download script required.*
...
cat("Date range:", as.character(min(panel_station_day$date, na.rm = TRUE)), "to", as.character(max(panel_station_day$date, na.rm = TRUE)), "\n")
...
For users who want to skip the data wrangling process and jump directly into analysis, we provide pre-built Parquet files in the `data/derived/` directory. These files contain the fully processed panel datasets ready for modeling and visualization.

## Quick Start

To load the prepared datasets, simply run:

    library(arrow)

    # Load the station-day panel (recommended for most analyses)
    panel_day <- read_parquet("data/derived/panel_station_day_indexed.parquet")

    # Load the station-hour panel (for hourly/intraday analyses)
    panel_hour <- read_parquet("data/derived/panel_station_hour_indexed.parquet")

## Available Files

| File | Description | Rows | Use Case |
|------|-------------|------|----------|
| `panel_station_day_indexed.parquet` | Station-day level ridership with weather, events, and baseline | ~85K | Daily impact analysis, regression models |
| `panel_station_hour_indexed.parquet` | Station-hour level ridership with event flags | ~2M | Hourly patterns, intraday event effects |
| `station_lookup.csv` | Station metadata (names, coordinates) | ~120 | Reference table |
| `station_summary.csv` | Aggregated station statistics | ~120 | Quick station comparisons |

## Key Variables

**Station-Day Panel (`panel_station_day_indexed.parquet`):**

*Core identifiers:*

- `station_complex_id`: Unique station identifier
- `station_complex`: Human-readable station name
- `date`: Date of observation

*Ridership:*

- `entries_day`: Total daily ridership at station
- `baseline_sd`: Median ridership for this station/day-of-week/month
- `pct_change`: Log percent change from baseline
...
- `heatwave`: Binary flag (1 = part of 3+ day heat spell ≥90°F)
- `storm`: Binary flag (1 = precipitation ≥1 inch)

*Event & calendar:*

- `event_day`: Binary flag (1 = nearby event active)
- `is_holiday`: "Yes"/"No" for federal holidays
- `dow`: Day of week (Mon-Sun)
- `month`: Month number (1-12)

**Station-Hour Panel (`panel_station_hour_indexed.parquet`):**
...
- `total_ridership`: Hourly ridership count
- `baseline_sh`: Median ridership for this station/dow/month/hour
- `hourly_pct_change`: Log percent change from baseline

*Event flag:*

- `event_hour`: Binary flag (1 = event active during this hour)

## Example Usage

**Analyze heatwave impacts on ridership:**

```{r example_heatwave, eval=FALSE}
panel_station_day |>
  filter(is_holiday == "No") |>  # Exclude holidays
  group_by(heatwave) |>
  summarize(
    n_obs = n(),
    mean_ridership = mean(entries_day, na.rm = TRUE),
    mean_pct_change = mean(pct_change, na.rm = TRUE)
  )
```

**Compare event vs. non-event hours:**

```{r example_events, eval=FALSE}
panel_station_hour |>
  group_by(event_hour) |>
  summarize(
    n_obs = n(),
    mean_ridership = mean(total_ridership, na.rm = TRUE),
    mean_pct_change = mean(hourly_pct_change, na.rm = TRUE)
  )
```

**Filter to specific stations or date ranges:**

```{r example_filter, eval=FALSE}
# Times Square station during summer 2024
panel_station_day |>
  filter(
    str_detect(station_complex, "Times Sq"),
    date >= "2024-06-01",
    date <= "2024-08-31"
  )
```
...
- **Use Parquet**: Parquet files load 5-10x faster than CSV and use less memory
- **Filter early**: Apply `filter()` before `summarize()` to reduce computation
- **Choose the right granularity**: Use station-day for most analyses; only load station-hour when hourly resolution is required
