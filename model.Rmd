...
\[
\text{Ridership} \sim s(\text{Temperature}) + s(\text{Precipitation}) + s(\text{Snowfall}) + \text{Day of Week} + \text{Month}
\]
...
# Download NYC neighborhood tabulation areas for detailed boundaries
# This provides much more detailed geography than borough outlines
nta_url <- "https://data.cityofnewyork.us/api/geospatial/9nt8-h7nd?method=export&format=GeoJSON"
nyc_nta <- tryCatch({
  st_read(nta_url, quiet = TRUE)
}, error = function(e) {
  # Fallback to borough boundaries
...
# 3. Rain Sensitivity Analysis - Hourly basis (for Story 4)
